...
# Signal preprocessing (optional but recommended)
from scipy import signal
from sklearn.preprocessing import StandardScaler, RobustScaler, MinMaxScaler

def preprocess_emg_signals(data, fs=1024, lowcut=20, highcut=450, notch_freq=50, 
                           q=30, normalization=None, plot_example=False):
    """
    Preprocess raw EMG signals.
    
    Args:
        data: Raw EMG data with shape (sessions, electrodes, time)
        fs: Sampling frequency in Hz (default: 1024 Hz as per project description)
        lowcut: Lower cutoff frequency for bandpass filter in Hz
        highcut: Higher cutoff frequency for bandpass filter in Hz
        notch_freq: Frequency to filter out (typically power line frequency, 50/60 Hz)
        q: Quality factor for notch filter
        normalization: Type of normalization ('standard', 'robust', 'minmax', or None)
        plot_example: Whether to plot an example of preprocessing effect
    
    Returns:
        Preprocessed EMG data with the same shape as input data
    """
    ...
    # Example channel for plotting
    example_session, example_electrode = 0, 0
    ...
    for s in range(n_sessions):
        for e in range(n_electrodes):
            ...
            # Save original signal for plotting if requested
            if plot_example and s == example_session and e == example_electrode:
                original_signal = raw_signal.copy()
                
            # Apply bandpass filter
            filtered_signal = signal.filtfilt(b_bandpass, a_bandpass, raw_signal)
            
            # Apply notch filter
            filtered_signal = signal.filtfilt(b_notch, a_notch, filtered_signal)
            
            # Store the filtered signal
            processed_data[s, e, :] = filtered_signal
    
    # Normalization (if requested)
    if normalization:
        # Reshape for normalization
        orig_shape = processed_data.shape
        reshaped_data = processed_data.reshape(-1, n_samples)
        ...
        processed_data = normalized_data.reshape(orig_shape)
    
    # Plot an example of preprocessing effect if requested
    if plot_example and original_signal is not None:
        processed_signal = processed_data[example_session, example_electrode, :]
        ...
        plt.plot(t[:1000], original_signal[:1000], label='Original')
        plt.plot(t[:1000], processed_signal[:1000], label='Processed')
        plt.xlabel('Time (s)')
        plt.ylabel('Amplitude')
        plt.title('Time Domain (1 second sample)')
        plt.legend()
        
        # Plot full spectrum
        plt.subplot(2, 2, 2)
        f_orig, psd_orig = signal.welch(original_signal, fs, nperseg=2048)
        f_proc, psd_proc = signal.welch(processed_signal, fs, nperseg=2048)
        plt.semilogy(f_orig, psd_orig, label='Original')
        plt.semilogy(f_proc, psd_proc, label='Processed')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('PSD (V^2/Hz)')
        plt.title('Power Spectral Density')
        plt.legend()
        
        # Zoom in on lower frequencies to see notch effect
        plt.subplot(2, 2, 3)
        plt.semilogy(f_orig[f_orig <= 100], psd_orig[f_orig <= 100], label='Original')
        plt.semilogy(f_proc[f_proc <= 100], psd_proc[f_proc <= 100], label='Processed')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('PSD (V^2/Hz)')
        plt.title('PSD (Zoomed to 0-100 Hz)')
        plt.axvline(x=notch_freq, color='r', linestyle='--', label=f'{notch_freq} Hz')
        plt.legend()
        
        # Plot histogram to see distribution changes
        plt.subplot(2, 2, 4)
        plt.hist(original_signal, bins=50, alpha=0.5, label='Original')
        plt.hist(processed_signal, bins=50, alpha=0.5, label='Processed')
...
# Preprocess guided gestures data
print("Preprocessing guided gestures data...")
guided_X_processed = preprocess_emg_signals(
    guided_X,
    fs=1024,  # Sampling frequency from project description
    lowcut=20,  # EMG signals typically start around 20 Hz
    highcut=450,  # High frequency cutoff to remove noise
    notch_freq=50,  # European power line frequency (use 60 for North America)
    q=30,  # Quality factor for notch filter
    normalization='robust',  # Robust normalization to handle outliers
    plot_example=True  # Plot an example to visualize the effect
)

# Preprocess free gestures data
print("\nPreprocessing free gestures data...")
free_X_processed = preprocess_emg_signals(
    free_X,
    fs=1024,
    lowcut=20,
    highcut=450,
    ...
    plot_example=False  # No need to plot again
)

# Preprocess test data
print("\nPreprocessing test data...")
# We need to reshape test data first
guided_test_X_reshaped = guided_test_X.reshape(5, 8, -1)  # Reshape to (sessions, electrodes, time)
free_test_X_reshaped = free_test_X.reshape(5, 8, -1)

# Apply preprocessing
guided_test_X_processed = preprocess_emg_signals(
    guided_test_X_reshaped,
    fs=1024,
    lowcut=20,
    highcut=450,
    notch_freq=50,
...
free_test_X_processed = preprocess_emg_signals(
    free_test_X_reshaped,
    fs=1024,
    lowcut=20,
    highcut=450,
    notch_freq=50,
...
guided_test_X_processed = guided_test_X_processed.reshape(guided_test_X.shape)
free_test_X_processed = free_test_X_processed.reshape(free_test_X.shape)

print("\nPreprocessing complete.")
print(f"Guided gestures processed data shape: {guided_X_processed.shape}")
print(f"Free gestures processed data shape: {free_X_processed.shape}")
print(f"Guided test processed data shape: {guided_test_X_processed.shape}")
print(f"Free test processed data shape: {free_test_X_processed.shape}")

# Use the processed data for the rest of the analysis
guided_X = guided_X_processed
free_X = free_X_processed
guided_test_X = guided_test_X_processed
free_test_X = free_test_X_processed
```

Now, let's create a function to extract windows with overlap:

```python
def create_windows(data, target, window_size=500, overlap=0.75):
    """
    Create overlapping windows from continuous data.
    
    Args:
        data: EMG data with shape (sessions, electrodes, time)
        target: Target data with shape (sessions, joints, time)
        window_size: Size of each window
        overlap: Overlap between consecutive windows (0.0 to 1.0)
    
    Returns:
        Windows of EMG data and corresponding target data (last frame)
    """
    ...
    for session in range(n_sessions):
        ...
        for i in range(0, n_samples - window_size + 1, step):
            # Extract window from the EMG data
            emg_window = data[session, :, i:i+window_size]
            X_windows.append(emg_window)
            
            # Extract the last frame of the target window
            # This represents the hand pose at the end of the EMG measurement
            target_frame = target[session, :, i+window_size-1]
            y_windows.append(target_frame)
            
            session_windows += 1
        
        total_windows += session_windows
        print(f"Session {session+1}: {session_windows} windows created")
...
```python
# Create windows with 75% overlap using the last frame as target
print("\nCreating windows with 75% overlap...")
X_guided_windows, y_guided_windows = create_windows(
    guided_X, guided_y, window_size=500, overlap=0.75
)

X_free_windows, y_free_windows = create_windows(
    free_X, free_y, window_size=500, overlap=0.75
)
...
# Visualize a window to understand the data
plt.figure(figsize=(15, 8))

# Plot EMG signals for all 8 electrodes in one window
plt.subplot(2, 1, 1)
for i in range(8):
    plt.plot(X_guided_windows[0, i, :], label=f'Electrode {i+1}')
plt.title('EMG Signals from All Electrodes (One Window)')
plt.xlabel('Sample')
plt.ylabel('Amplitude')
plt.legend(loc='upper right', bbox_to_anchor=(1.15, 1))

# Plot corresponding joint angles (first 10 of 51 for clarity)
plt.subplot(2, 1, 2)
plt.bar(range(10), y_guided_windows[0, :10], color='green')
plt.title('First 10 Joint Angles (Target Values - Last Frame)')
...
# Analyze timing with 75% overlap
window_size = 500
overlap = 0.75
step_size = int(window_size * (1 - overlap))
window_duration_sec = window_size / 1024  # in seconds
step_duration_sec = step_size / 1024  # in seconds

print(f"\nWindow Analysis with {overlap*100}% overlap:")
print(f"Window size: {window_size} samples = {window_duration_sec:.3f} seconds")
print(f"Step size: {step_size} samples = {step_duration_sec:.3f} seconds")
print(f"This means a new prediction every {step_duration_sec*1000:.1f} milliseconds")
```
